...
$(function () {
    ...
    var Translation = /** @class */ (function () {
        ...
        Translation.prototype.setLanguage = function (lang) {
            this.language = lang;
        };
        Translation.prototype.getLanguage = function () {
        ...
        Translation.prototype.get = function (text, lang) {
        ...
        Translation.prototype.perform = function (lang) {
            if (typeof lang === "undefined")
                lang = this.language;
            var self = this;
    ...
    var CanvasRenderer = /** @class */ (function (_super) {
        ...
        CanvasRenderer.prototype.init = function (piano) {
            ...
            _super.prototype.init.call(this, piano);
            // create render loop
            var self = this;
            var render = function () {
                self.redraw();
        ...
        CanvasRenderer.prototype.redraw = function (width, height) {
        ...
        CanvasRenderer.prototype.isSupported = function () {
            var canvas = document.createElement("canvas");
    ...
    var SoundSelector = /** @class */ (function () {
        ...
        SoundSelector.prototype.loadPack = function (pack, f) {
            ...
            for (var k in this.piano.keys) {
                if (!this.piano.keys.hasOwnProperty(k))
                    continue;
                (function () {
    ...
    Piano.prototype.play = function (note, vol, participant, delay_ms, lyric) {
        ...
        if (gMidiOutTest)
            window.gMidiOutTest(key.note, vol * 100, delay_ms);
        var self = this;
        setTimeout(function () {
    ...
    Piano.prototype.stop = function (note, participant, delay_ms) {
        ...
        if (gMidiOutTest)
            window.gMidiOutTest(key.note, 0, delay_ms);
    };
    ...
    function press(id, vol) {
        if (!window.gClient.preventsPlaying() && window.gNoteQuota.spend(1)) {
            gHeldNotes[id] = true;
    ...
    (function () {
        gClient.on("participant added", function (part) {
            ...
            part.displayY = 50;
            // add nameDiv
            var div; // TODO: make this not any, if possible
            ...
            // sort names
            var arr; // TODO: find a way to make this some sort of JQuery array instead of any
            arr = $("#names .name");
    ...
    gClient.on("ch", function (msg) {
        ...
        var notice_div = $("#room-notice");
        if (has_notice) {
            notice_div.html(notice);
            if (notice_div.is(':hidden'))
                notice_div.fadeIn(1000);
        }
        else {
            if (notice_div.is(':visible'))
                notice_div.fadeOut(1000);
        }
    });
    gClient.on("disconnect", function () {
    ...
    var gPianoMutes = (localStorage.pianoMutes ? localStorage.pianoMutes : "").split(',').filter(function (v) { return v; });
    var gChatMutes = (localStorage.pianoMutes ? localStorage.pianoMutes : "").split(',').filter(function (v) { return v; });
    var volume_slider; // TODO: make this not any
    ...
    function handleKeyDown(evt) {
        //console.log(evt);
        var code;
        code = parseInt(evt.keyCode); //! Deprecated - Hri7566
    ...
    function handleKeyUp(evt) {
        var code;
        var code = parseInt(evt.keyCode); //! Also deprecated - Hri7566
        if (key_binding[code] !== undefined) {
            var binding = key_binding[code];
            if (binding.held) {
                binding.held = false;
    ...
    var Notification = /** @class */ (function (_super) {
        ...
        function Notification(par) {
            ...
            if (_this instanceof Notification === false)
                throw ("yeet");
            var ipar = par || {};
            _this.id = "Notification-" + (par.id || Math.random());
            _this.title = par.title || "";
            _this.text = par.text || "";
            _this.html = par.html || "";
            _this.target = $(par.target || "#piano");
            _this.duration = par.duration || 30000;
            _this["class"] = par["class"] || "classic";
            var self = _this;
            var eles = $("#" + _this.id);
            if (eles.length > 0) {
                eles.remove();
            }
            _this.domElement = $('<div class="notification"><div class="notification-body"><div class="title"></div>' +
                '<div class="text"></div></div><div class="x">Ⓧ</div></div>');
            _this.domElement[0].id = _this.id;
            _this.domElement.addClass(_this["class"]);
            _this.domElement.find(".title").text(_this.title);
            if (_this.text.length > 0) {
                _this.domElement.find(".text").text(_this.text);
            }
            else if (_this.html instanceof HTMLElement) {
                _this.domElement.find(".text")[0].appendChild(_this.html);
            }
            else if (_this.html.length > 0) {
                _this.domElement.find(".text").html(_this.html);
            }
            document.body.appendChild(_this.domElement.get(0));
            _this.position();
            _this.onresize = function () {
                self.position();
            };
            window.addEventListener("resize", _this.onresize);
            _this.domElement.find(".x").click(function () {
                self.close();
            });
            if (_this.duration > 0) {
                setTimeout(function () {
        ...
        Notification.prototype.onresize = function () {
        };
    ...
    if (!gKnowsYouCanUseKeyboard) {
        window.gKnowsYouCanUseKeyboardTimeout = setTimeout(function () {
            window.gKnowsYouCanUseKeyboardNotification = new Notification({ title: "Did you know!?!", text: "You can play the piano with your keyboard, too.  Try it!", target: "#piano", duration: 10000 });
        }, 30000);
    }
    if (window.localStorage) {
    ...
    gClient.on("ch", function (msg) {
        ...
        if (!channel.settings.chat)
            info.addClass("no-chat");
        else
            info.removeClass("no-chat");
        if (channel.settings.crownsolo)
            info.addClass("crownsolo");
        else
            info.removeClass("crownsolo");
        if (channel.settings['no cussing'])
            info.addClass("no-cussing");
        else
            info.removeClass("no-cussing");
        if (!channel.settings.visible)
            info.addClass("not-visible");
        else
            info.removeClass("not-visible");
    });
    gClient.on("ls", function (ls) {
        for (var i in ls.u) {
            ...
            if (!room.settings.visible)
                ...
                info.removeClass("not-visible");
            if (room.banned)
                info.addClass("banned");
            else
                info.removeClass("banned");
        }
    });
    ...
    $(window).on("popstate", function (evt) {
        ...
        if (depth == gHistoryDepth)
            return; // <-- forgot why I did that though...
    ...
    var chat = (function () {
        ...
        $(document).mousedown(function (evt) {
            if (!($("#chat").has(evt.target).length > 0)) { //! make this not any - Hri7566
                chat.blur();
            }
        });
        document.addEventListener("touchstart", function (event) {
            for (var i in event.changedTouches) {
                ...
                if (!($("#chat").has(touch.target).length > 0)) { //! same thing here - Hri7566
                    chat.blur();
                }
            }
        });
        ...
        $("#chat input").on("keydown", function (evt) {
            if (evt.keyCode == 13) {
                if (window.MPP.client.isConnected()) {
        ...
        return {
            ...
            receive: function (msg) {
                if (gChatMutes.indexOf(msg.p._id) != -1)
                    return;
                ...
                $("#chat ul").append(li);
                var eles = $("#chat ul li").get(); // TODO: Make this not any
            ...
            navigator.requestMIDIAccess().then(function (midi) {
                ...
                function plug() {
                    ...
                    if (midi.outputs.size > 0) {
                        ...
                        window.gMidiOutTest = function (note_name, vel, delay_ms) {
                ...
                function showConnections(sticky) {
                    ...
                    if (midi.inputs.size > 0) {
                        ...
                        for (var input_it = inputs.next(); input_it && !input_it.done; input_it = inputs.next()) {
                            var input = input_it.value;
                            var li = document.createElement("li"); // TODO: make this not any - hri
                            ...
                            if (gMidiVolumeTest) {
                                var knob = document.createElement("canvas"); // TODO: this uses any for now, change it later
    ...
    window.onerror = function (message, url, line) {
        ...
        if (url.indexOf("socket.io.js") !== -1) {
            if (message.indexOf("INVALID_STATE_ERR") !== -1)
                return;
            if (message.indexOf("InvalidStateError") !== -1)
                return;
            if (message.indexOf("DOM Exception 11") !== -1)
                return;
            ...
            if (message.indexOf("Cannot call method 'payload' of null") !== -1)
                return;
            if (message.indexOf("Unable to get value of the property 'close'") !== -1)
                return;
            if (message.indexOf("NS_ERROR_NOT_CONNECTED") !== -1)
                return;
            if (message.indexOf("Unable to get property 'close' of undefined or null reference") !== -1)
                return;
            if (message.indexOf("Unable to get value of the property 'close': object is null or undefined") !== -1)
                return;
            if (message.indexOf("this.transport is null") !== -1)
                return;
        }
        // errors in soundmanager2
        if (url.indexOf("soundmanager2.js") !== -1) {
            // operation disabled in safe mode?
            if (message.indexOf("Could not complete the operation due to error c00d36ef") !== -1)
                return;
        ...
        if (url.indexOf("script.js") !== -1) {
            if (message.indexOf("Object [object Object] has no method 'on'") !== -1)
                return;
            if (message.indexOf("Object [object Object] has no method 'off'") !== -1)
                return;
            if (message.indexOf("Property '$' of object [object Object] is not a function") !== -1)
                return;
        }
        var enc = "/bugreport/"
    ...
    (function () {
        ...
        var audio = window.MPP.piano.audio;
    ...
    (function () {
        ...
        function showSynth() {
            ...
            // mix
            var knob = document.createElement("canvas"); // TODO replace any
